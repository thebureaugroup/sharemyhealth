...
def test_patient_everything_bundle(request):
    ...
    oas = OAuth2Session(
        request.session['client_id'], token=request.session['token'])
    pe_uri = "%s/hie/api/fhir/stu3/Patient/$everything" % (request.session['resource_uri'])
    print(pe_uri)
...
def test_patient(request):
    ...
    oas = OAuth2Session(
        request.session['client_id'], token=request.session['token'])
    patient_uri = "%s/fhir/baseDstu3/Patient/%s" % (
...
def test_condition(request):
    ...
    oas = OAuth2Session(
        request.session['client_id'], token=request.session['token'])
    uri = "%s/fhir/baseDstu3/Condition/?subject=%s" % (
        request.session['resource_uri'], request.session['patient'])
